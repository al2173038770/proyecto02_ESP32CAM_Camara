...
void loop() {
  ...
  if (timeNow - timeLast > wait) { // Manda un mensaje por MQTT cada cinco segundos
    ...
    client.publish("esp32/data/grup2", dataString); // Esta es la función que envía los datos por MQTT, especifica el tema y el valor
...
void callback(char* topic, byte* message, unsigned int length) {
  ...
  if (String(topic) == "esp32/data/grup2") {  // En caso de recibirse mensaje en el tema esp32/output
    if(messageTemp == "true"){
...
void reconnect() {
  ...
  while (!client.connected()) { // Pregunta si hay conexión
    ...
    if (client.connect("ESP32CAMClient")) { //Pregunta por el resultado del intento de conexión
      ...
      client.subscribe("esp32/data/grup2"); // Esta función realiza la suscripción al tema
